...
Select.accessible = React.createClass({
  ...
  buildMenu: function () {
    ...
    this.cachedListItemsIndexLookup = {};
      ...
      if(isFocused)
      {
        ...
        checkMark = " Selected";
			...
			return <div role="button option" aria-label={op.label + checkMark} ref={ref} key={'option-' + op.value} className={optionClass} onMouseEnter={mouseEnter} onMouseLeave={mouseLeave} onMouseDown={mouseDown} onClick={mouseDown}>{op.label}</div>;
	...
	render: function() {
		...
		if (!this.state.inputValue && (!this.props.multi || !value.length)) {
			if(this.state.placeholder === this.props.placeholder)
				value.push(<div aria-label={this.state.placeholder + " " + this.state.filteredOptions.length + " available options"} className="Select-placeholder" key="placeholder">{this.state.placeholder}</div>);
			else
				value.push(<div aria-label={this.state.placeholder + " is currently selected"} className="Select-placeholder" key="placeholder">{this.state.placeholder}</div>);
		}
    ...
    this.cachedMenu = builtMenu;
		...
		var menu = 1 ? <div id="Select-menu" role="group" ref="menu" className="Select-menu">{builtMenu}</div> : null;

		return (
			<div ref="wrapper" className={selectClass}>
				<input type="hidden" ref="value" name={this.props.name} value={this.state.value} />
				<div role="button listbox" aria-owns="Select-menu" className="Select-control" ref="control" onKeyDown={this.handleKeyDown} onMouseDown={this.handleMouseDown} onTouchEnd={this.handleMouseDown}>
					{value}
					<Input className="Select-input" tabIndex={this.props.tabIndex} ref="input" value={this.state.inputValue} onFocus={this.handleInputFocus} onBlur={this.handleInputBlur} onChange={this.handleInputChange} minWidth="5" />
					<span className="Select-arrow" />
					{loading}
